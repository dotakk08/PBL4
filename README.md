...
### Bản cải tiến  (`msinet_caitien_decoder_fpn`)

Tích hợp **FPN (Feature Pyramid Network)** vào decoder để khai thác đặc trưng đa tỉ lệ tốt hơn.

### Bản cải tiến  (`msinet_caitien_encoder_efficientnetb0`)
...
Vào **Settings → Accelerator → GPU P100**.
...
weights/ (ngoại trừ với model sử dụng EfficientNetB0 sẽ không có)
